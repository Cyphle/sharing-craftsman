...
import fr.knowledge.infra.events.library.KnowledgeDeletedInfraEvent;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import java.io.IOException;

public class KnowledgeDeletedEventDeserializer implements EventDeserializer {
  private final Logger log = LoggerFactory.getLogger(this.getClass());

  @Override
  public DomainEvent deserialize(String event) {
    try {
      return Mapper.fromJsonStringToObject(event, KnowledgeDeletedInfraEvent.class).fromInfraToDomain();
    } catch (IOException e) {
  ...
  public String serialize(DomainEvent change) {
    try {
      return Mapper.fromObjectToJsonString(KnowledgeDeletedInfraEvent.fromDomainToInfra(change));
