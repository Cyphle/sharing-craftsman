...
public class CategoryUpdatedEventDeserializer implements EventDeserializer {
  private final Logger log = LoggerFactory.getLogger(this.getClass());

  @Override
  public DomainEvent deserialize(String event) {
    try {
      return Mapper.fromJsonStringToObject(event, CategoryUpdatedInfraEvent.class).fromInfraToDomain();
    } catch (IOException e) {
  ...
  public String serialize(DomainEvent change) {
    try {
      return Mapper.fromObjectToJsonString(CategoryUpdatedInfraEvent.fromDomainToInfra(change));
