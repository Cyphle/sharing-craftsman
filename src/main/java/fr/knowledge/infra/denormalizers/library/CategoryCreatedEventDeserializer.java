...
public class CategoryCreatedEventDeserializer implements EventDeserializer {
  private final Logger log = LoggerFactory.getLogger(this.getClass());

  @Override
  public DomainEvent deserialize(String event) {
    try {
      CategoryCreatedInfraEvent categoryCreatedInfraEvent = Mapper.fromJsonStringToObject(event, CategoryCreatedInfraEvent.class);
      return categoryCreatedInfraEvent.fromInfraToDomain();
    } catch (IOException e) {
  ...
  public String serialize(DomainEvent change) {
    try {
      return Mapper.fromObjectToJsonString(CategoryCreatedInfraEvent.fromDomainToInfra(change));
