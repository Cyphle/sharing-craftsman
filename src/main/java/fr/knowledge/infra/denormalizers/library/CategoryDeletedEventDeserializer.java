...
public class CategoryDeletedEventDeserializer implements EventDeserializer {
  private final Logger log = LoggerFactory.getLogger(this.getClass());

  @Override
  public DomainEvent deserialize(String event) {
    try {
      return Mapper.fromJsonStringToObject(event, CategoryDeletedInfraEvent.class).fromInfraToDomain();
    } catch (IOException e) {
  ...
  public String serialize(DomainEvent change) {
    try {
      return Mapper.fromObjectToJsonString(CategoryDeletedInfraEvent.fromDomainToInfra(change));
